...
Template.velocityReports.helpers({
  ...
  suiteTrees: function () {
    ...
    var suiteToTrees = new OrderedDict();
    _.each(reports, function(report) {
      var context = suiteToTrees;
      _.each(report.ancestors.reverse(), function(suite) {
        var suiteTree = context.get(suite);
        if(!suiteTree) {
          suiteTree = {
            name: suite,
            children: new OrderedDict()
          }
          context.append(suite, suiteTree);
        }
        context = suiteTree.children;
      });
      context.append(report.name, report);
    });
    var _serializeTree = function(suiteTree) {
      var serialized = null;
      if(suiteTree.children) {
        serialized = {
          name: suiteTree.name,
          itemType: "suite",
          children: []
        };
        suiteTree.children.forEach(function(tree, suite) {
          serialized.children.push(_serializeTree(tree));
        });
      } else {
        serialized = {
          name: suiteTree.name,
          passed: suiteTree.passed,
          itemType: "result"
        }
      }
      return serialized;
    ...
    var serializedTrees = [];
    suiteToTrees.forEach(function(tree, suite) {
      var serialized = _serializeTree(tree);
      serialized.collapsed = "collapsed";
      serializedTrees.push(serialized);
    });
...
Template.velocityReports.events({
  'click li.suite, span.name': function(e) {
    e.stopPropagation();
    var $target = $(e.target);
